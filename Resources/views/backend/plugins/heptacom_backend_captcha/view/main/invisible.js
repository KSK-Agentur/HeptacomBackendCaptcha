...
var reCaptchaOnSubmit = function(token) {
    ...
    form.submit({
        ...
        waitMsg: '{s name=wait/message namespace=backend/login/controller/main}Login...{/s}',
        ...
        failure: function(form, action) {
            ...
            if(action.result.lockedUntil) {
                ...
                message = "{s name=failure/locked_message namespace=backend/login/controller/main}Der Account ist bis zum [lockedUntil:date] um [lockedUntil:date('H:i:s')] Uhr gesperrt.{/s}";
                message = new Ext.Template(message);
                message = message.applyTemplate(action.result);
            } else {
                message = '{s name=failure/input_message namespace=backend/login/controller/main}Bitte überprüfen Sie Ihre Eingabe und probieren es erneut.{/s}';
            }
            Ext.Msg.alert('{s name=failure/title namespace=backend/login/controller/main}Login fehlgeschlagen{/s}', '{s name=failure/message namespace=backend/login/controller/main}Ihr Login war nicht erfolgreich. {/s}' + message);
            return false;
        }
    });
};
